...
def _analyze(board, mated):
    retval = {'ideal':False, 'model':False, 'pure':False, 'hash':0, 'traversal':-1,
        'mated':board.kings[mated], 'octet':False, 'pins':0}
    ...
    king_area = chess.BitBoard(chess.LUT.att[mated_king][board.kings[mated]].v[0], \
    ...
    for square in chess.SetBits(king_area):
        ...
        if (len(attackers[square]) == 1):
            if not chess.LUT.btw[attackers[square][0][1]][board.kings[mated]][square]:
                return retval
    ...
    for name, pos in chess.Pieces(board, mating):
        if (not name in ['pk', 'PK'][mating == chess.WHITE]) and (not participants[pos]):
            retval['model'] = False
            break
    ...
    for i in xrange(len(traversals)):
        ...
        for j in xrange(len(traversals[i])):
            ...
            if -1 == square: # board edge
                ...
                id_map, participant_id, next_id = inc_id(id_map, attackers[square][0][1], next_id)
...
class FinalesVisitor:
    ...
    def visit(self, node, board, side_on_move):
        if not isinstance(node, chess.MoveNode): return
        if isinstance(node.move, chess.NullMove): return
        if not (node.move.is_mate or node.move.is_stalemate): return
        info = analyze(board, side_on_move)
        if not info['pure']: return
        info['stale'] = node.move.is_stalemate
        
        if self.by_hash.has_key(info['hash']):
            self.by_hash[info['hash']].append(info)
        else:
            self.by_hash[info['hash']] = [info]
        
        extended_key = str(info['hash']) + str(info['mated']) + str(info['traversal'])
        if self.different.has_key(extended_key):
            self.different[extended_key].append(info)
        else:
            self.different[extended_key] = [info]
        
def provides():
    return ['Model mates', 'Model stalemates', 'Ideal mates', 'Ideal satelemates', 'Echo', 'Chameleon echo',\
            'Octet', 'Models with pin', 'Models with two pins', 'Models with three pins']
...
def check(problem, board, solution):
    ...
    retval = {}
    for keyword in provides(): retval[keyword] = False
    
    mm, msm, im, ism = 0, 0, 0, 0
    for dif in visitor.different:
        mm_, msm_, im_, ism_ = 0, 0, 0, 0
        for info in visitor.different[dif]:
            if info['model'] and info['stale']: msm_ = 1
            if info['model'] and not info['stale']: mm_ = 1
            if info['ideal'] and info['stale']: ism_ = 1
            if info['ideal'] and not info['stale']: im_ = 1
            if info['octet']: retval['Octet'] = True
            if info['pins'] > 0: retval['Models with pin'] = True
            if info['pins'] > 1: retval['Models with two pins'] = True
            if info['pins'] > 2: retval['Models with three pins'] = True
        mm, msm, im, ism = mm + mm_, msm + msm_, im + im_, ism + ism_
    retval['Model mates'], retval['Model stalemates'], retval['Ideal mates'], retval['Ideal satelemates'] =\
        mm > 1, msm > 1, im > 1, ism > 1
    ...
    for hash in visitor.by_hash:
        if len(visitor.by_hash[hash]) < 2: continue
        for info1, info2 in common.tuples(visitor.by_hash[hash], 2, False):
            if info1['mated'] <> info2['mated'] or info1['traversal'] <> info2['traversal']:
                retval['Echo'] = True
